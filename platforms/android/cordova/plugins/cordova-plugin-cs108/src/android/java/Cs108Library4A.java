...
public class Cs108Library4A extends Cs108Connector {
    ...
    @Keep
    public Cs108Library4A(Context context) {
        super(context);
...
//                    if (matched) appendToLogView("feature = " + f.name);
    ...
    @Override
    @Keep
    public boolean scanLeDevice(final boolean enable, BluetoothAdapter.LeScanCallback mLeScanCallback, ScanCallback mScanCallBack) {
...
//        mUsbConnector.scanUsbDevice(enable);
    ...
    public String getBluetoothDeviceName() {
        if (getmBluetoothDevice() == null) return null;
    ...
    public boolean isBleConnected() {
        ...
        if (bleConnectionNew) {
            if (bleConnection == false) {
                ...
                getSerialNumber();
                regionCode = null;
                mRfidDevice.mRx000Device.mRx000OemSetting.getCountryCode();
    ...
    public boolean getRfidOnStatus() {
        return mRfidDevice.getOnStatus();
    }

    public boolean setRfidOn(boolean onStatus) {
        return mRfidDevice.mRx000Device.turnOn(onStatus);
    }

    @Keep
    public boolean isBarcodeFailure() {
        return mBarcodeDevice.barcodeFailure;
    }

    @Keep
    public boolean isRfidFailure() {
        return mRfidDevice.rfidFailure;
    }
    ...
    private final Runnable checkVersionRunnable = new Runnable() {
        ...
        public void run() {
            if (mRfidDevice.mRx000Device.mRx000Setting.getFreqChannelConfig() < 0 || (isBarcodeFailure() == false && mBarcodeDevice.bBarcodeTriggerMode == (byte) 0xFF)) {
                if (DEBUG)
                    appendToLog("checkVersionRunnable(): RESTART with FreqChannelConfig = " + mRfidDevice.mRx000Device.mRx000Setting.getFreqChannelConfig() + ", bBarcodeTriggerMode = " + mBarcodeDevice.bBarcodeTriggerMode);
                mHandler.removeCallbacks(checkVersionRunnable);
                mHandler.postDelayed(checkVersionRunnable, 500);
            } else {
                ...
                if (isBarcodeFailure() == false) {
                    ...
                    getAutoRFIDAbort();
                    getAutoBarStartSTop(); //setAutoRFIDAbort(false); setAutoBarStartSTop(true);
                }
        ...
        if (file.exists()) {
            ...
            try {
                ...
                if (instream != null) {
                    ...
                    int retryCount = -1;
                    int fixedQValue = -1;
                    int fixedRetryCount = -1;
                    int population = -1;
                    boolean invAlgo = true;
                    int retry = -1;
                    while ((line = bufferedReader.readLine()) != null) {
                        ...
                        if (dataArray.length == 2) {
                            if (dataArray[0].matches("appVersion")) {
                                ...
                                if (DEBUG)
                                    appendToLog("loaded countryInList = " + Integer.valueOf(dataArray[1]));
                                getRegionList();
                                if (regionCode == null) {
                                ...
                                if (countryInList != countryInListNew)
                                    setCountryInList(countryInListNew);
                                channelOrderType = -1;
...
//                                appendToLogView("inventoryBeep = " + dataArray[1]);
                    ...
                    setTagFocus(tagFocus > 0 ? true : false);
    ...
    public String getMacVer() {
        return mRfidDevice.mRx000Device.mRx000Setting.getMacVer();
    }

    //getMacLastCommandDuration -- to be removed
    @Keep
    public long getMacLastCommandDuration(boolean request) {
        return mRfidDevice.mRx000Device.mRx000Setting.getMacLastCommandDuration(request);
    }
    ...
    public void resetEnvironmentalRSSI() {
        mRfidDevice.mRx000Device.mRx000EngSetting.resetRSSI();
    }
    ...
    public int getHighCompression() {
        return mRfidDevice.mRx000Device.mRx000MbpSetting.getHighCompression();
    }

    public int getRflnaGain() {
        return mRfidDevice.mRx000Device.mRx000MbpSetting.getRflnaGain();
    }
    ...
    public int getAgcGain() {
        return mRfidDevice.mRx000Device.mRx000MbpSetting.getAgcGain();
    }

    public int getRxGain() {
        return mRfidDevice.mRx000Device.mRx000MbpSetting.getRxGain();
    }

    public boolean setRxGain(int highCompression, int rflnagain, int iflnagain, int agcgain) {
        return mRfidDevice.mRx000Device.mRx000MbpSetting.setRxGain(highCompression, rflnagain, iflnagain, agcgain);
    }

    public boolean setRxGain(int rxGain) {
        return mRfidDevice.mRx000Device.mRx000MbpSetting.setRxGain(rxGain);
    }
...
/*                int[] freqTableIdx = fccFreqTableIdx;
    ...
    @Keep
    public boolean setConnectionHSpeed(boolean on) {
    ...
    public boolean setTam1Configuration(int keyId, String matchData) {
        ...
        String preChallenge = "00";
        preChallenge += String.format("%02X", keyId);
        matchData = preChallenge + matchData;
        retValue = setAuthMatchData(matchData);
        if (retValue)
            retValue = mRfidDevice.mRx000Device.mRx000Setting.setHST_AUTHENTICATE_CFG(true, true, matchData.length() * 4);
        return retValue;
    }
...
//    String serverLocation = "http://ptsv2.com/t/10i1t-1519143332/post";
    ...
    public int getStartQValue() {
        return mRfidDevice.mRx000Device.mRx000Setting.getAlgoStartQ(3);
    }

    @Keep
    public int getMaxQValue() {
        return mRfidDevice.mRx000Device.mRx000Setting.getAlgoMaxQ(3);
    }

    @Keep
    public int getMinQValue() {
        return mRfidDevice.mRx000Device.mRx000Setting.getAlgoMinQ(3);
    }

    public int getRetryCount() {
        int algoSelect = mRfidDevice.mRx000Device.mRx000Setting.getAlgoSelect();
        if (algoSelect == 0 || algoSelect == 3)
            return mRfidDevice.mRx000Device.mRx000Setting.getAlgoRetry(algoSelect);
        else return -1;
    }
    ...
    public boolean setDynamicQParms(int startQValue, int minQValue, int maxQValue, int retryCount) {
        ...
        if (result)
            result = mRfidDevice.mRx000Device.mRx000Setting.setAlgoStartQ(startQValue, maxQValue, minQValue, -1, -1, -1);
    ...
    public int getFixedQValue() {
        return mRfidDevice.mRx000Device.mRx000Setting.getAlgoStartQ(0);
    }

    @Keep
    public int getFixedRetryCount() {
        return mRfidDevice.mRx000Device.mRx000Setting.getAlgoRetry(0);
    }

    @Keep
    public boolean getRepeatUnitNoTags() {
        return mRfidDevice.mRx000Device.mRx000Setting.getAlgoRunTilZero(0) == 1 ? true : false;
    }

    @Keep
    public boolean setFixedQParms(int qValue, int retryCount, boolean repeatUnitNoTags) {
        if (DEBUG)
            appendToLog("qValue=" + qValue + ", retryCount = " + retryCount + ", repeatUntilNoTags = " + repeatUnitNoTags);
        ...
        if (result)
            result = mRfidDevice.mRx000Device.mRx000Setting.setAlgoStartQ(qValue, -1, -1, -1, -1, -1);
        if (result) result = mRfidDevice.mRx000Device.mRx000Setting.setAlgoRetry(retryCount);
        if (result)
            result = mRfidDevice.mRx000Device.mRx000Setting.setAlgoRunTilZero(repeatUnitNoTags ? 1 : 0);
        return result;
    }
    ...
    public boolean setSelectCriteria(int index, boolean enable, int target, int action, int delay, int bank, int offset, String mask) {
        ...
        if (index == 0)
            preMatchData = new PreMatchData(enable, target, action, bank, offset, mask, mask.length() * 4, mRfidDevice.mRx000Device.mRx000Setting.getQuerySelect(), getPwrlevel(), getInvAlgo(), getQValue());
        boolean result = true;
        if (index != mRfidDevice.mRx000Device.mRx000Setting.getInvSelectIndex())
            result = mRfidDevice.mRx000Device.mRx000Setting.setInvSelectIndex(index);
        if (result)
            result = mRfidDevice.mRx000Device.mRx000Setting.setSelectEnable(enable ? 1 : 0, target, action, delay);
        ...
        if (result)
            result = mRfidDevice.mRx000Device.mRx000Setting.setSelectMaskLength(mask.length() * 4);
    ...
    public boolean setSelectCriteria(int index, boolean enable, int target, int action, int delay, int bank, int offset, String mask, int maskblen) {
        ...
        int maskbytelen = maskblen / 4;
        if ((maskblen % 4) != 0) maskbytelen++;
        if (maskbytelen > 64) maskbytelen = 64;
        if (mask.length() > maskbytelen) mask = mask.substring(0, maskbytelen);
        if (index == 0)
            preMatchData = new PreMatchData(enable, target, action, bank, offset, mask, maskblen, mRfidDevice.mRx000Device.mRx000Setting.getQuerySelect(), getPwrlevel(), getInvAlgo(), getQValue());
        boolean result = true;
        if (index != mRfidDevice.mRx000Device.mRx000Setting.getInvSelectIndex())
            result = mRfidDevice.mRx000Device.mRx000Setting.setInvSelectIndex(index);
        if (result)
        ...
        if (result) result = mRfidDevice.mRx000Device.mRx000Setting.setSelectMaskOffset(offset);
        if (mask == null) return false;
        if (result) result = mRfidDevice.mRx000Device.mRx000Setting.setSelectMaskLength(maskblen);
        if (result) result = mRfidDevice.mRx000Device.mRx000Setting.setSelectMaskData(mask);
        if (result) {
            if (enable) {
                mRfidDevice.mRx000Device.mRx000Setting.setTagSelect(1);
    ...
    public boolean setPostMatchCriteria(boolean enable, boolean target, int offset, String mask) {
        ...
        if (result && mask != null)
            result = mRfidDevice.mRx000Device.mRx000Setting.setInvMatchData(mask);
        return result;
    }
...
//                        if (true)
//                            appendToLogView("getConnectionHSpeed() =  " + getConnectionHSpeed());
                    }
                }
    ...
    public void restoreAfterTagSelect() {
        if (DEBUG)
            appendToLog("postMatchDataChanged = " + postMatchDataChanged + ",  preMatchDataChanged = " + preMatchDataChanged + ", macVersion = " + getMacVer());
        if (checkHostProcessorVersion(getMacVer(), 2, 6, 8)) {
        ...
        if (postMatchDataChanged) {
            ...
            appendToLog("Hello6: invAlgo = " + postMatchDataOld.invAlgo);
        ...
        if (preMatchDataChanged) {
            ...
            appendToLog("Hello6: invAlgo = " + preMatchDataOld.invAlgo);
    ...
    boolean postMatchDataChanged = false;
    PreMatchData preMatchDataOld;
    boolean preMatchDataChanged = false;
    long pwrlevelOld;
    final boolean tagSelectByMatching = false;

    @Keep
    boolean setSelectedTag1(String selectMask, int selectBank, int selectOffset, long pwrlevel, int qValue, int matchRep) {
...
//            appendToLogView("Setting setSelectedTag1");
        ...
        if (setSuccess)
            setSuccess = mRfidDevice.mRx000Device.mRx000Setting.setTagDelay(tagDelayDefaultNormalSetting);
        if (setSuccess)
            setSuccess = mRfidDevice.mRx000Device.mRx000Setting.setCycleDelay(cycleDelaySetting);
        if (setSuccess)
            setSuccess = mRfidDevice.mRx000Device.mRx000Setting.setInvModeCompact(false);
        return setSuccess;
    }
...
//                break;
    ...
    Runnable runnableToggleConnection = new Runnable() {
        ...
        public void run() {
            if (DEBUG)
                appendToLog("runnableToggleConnection(): toggledConnection = " + toggledConnection + ", isBleConnected() = " + isBleConnected());
    ...
    public boolean setChannelHoppingStatus(boolean channelOrderHopping) {
        if (this.channelOrderType != (channelOrderHopping ? 0 : 1)) {
            ...
            for (int i = 0; i < freqcnt; i++) {
                ...
                if (result == true)
                    mRfidDevice.mRx000Device.mRx000Setting.setFreqChannelConfig(channelOrderHopping);
            }
            if (result == true)
                mRfidDevice.mRx000Device.mRx000Setting.setFreqChannelSelect(channel);
    ...
    public boolean setChannel(int channelSelect) {
        boolean result = true;
        if (result == true)
            result = mRfidDevice.mRx000Device.mRx000Setting.setFreqChannelConfig(false);
        if (result == true)
            result = mRfidDevice.mRx000Device.mRx000Setting.setFreqChannelSelect(channelSelect);
        if (result == true)
            result = mRfidDevice.mRx000Device.mRx000Setting.setFreqChannelConfig(true);
        return result;
    }
    ...
    public byte getPopulation2Q(int population) {
        ...
        if (DEBUG)
            appendToLog("getPopulation2Q(" + population + "): log dValue = " + dValue + ", iValue = " + iValue);
        return iValue;
    }
    ...
    public boolean getBarcodeOnStatus() {
        return mBarcodeDevice.getOnStatus();
    }
    ...
    public void getBarcodePreSuffix() {
        if (mBarcodeDevice.getPrefix() == null || mBarcodeDevice.getSuffix() == null)
...
//                if (retValue)
    ...
    public boolean setVibrateOn(int mode) {
        ...
        Cs108BarcodeData cs108BarcodeData = new Cs108BarcodeData();
        if (mode > 0)
            cs108BarcodeData.barcodePayloadEvent = BarcodePayloadEvents.BARCODE_VIBRATE_ON;
        ...
        if (start) {
            ...
            if (barcode2TriggerMode && result) {
                if (barcodeAutoStarted && result) {
                    ...
                    result = barcodeSendCommand(new byte[]{0x1b, 0x30});
                }
    ...
    public String getHostProcessorICBoardVersion() {
        ...
        if (str != null) {
            if (str.length() == 16) {
                ...
                if (str.substring(15, 16).matches("0") == false || strOut.length() < 3)
                    strOut += (strOut.length() < 3 ? "." : "") + str.substring(15, 16);
                return strOut;
            }
        }
        return null;
    ...
    */
    ...
    public boolean getAutoBarStartSTop() {
        return mNotificationDevice.getAutoBarStartStopStatus();
    }
    ...
    @Keep
    String getBatteryDisplay(boolean voltageDisplay, boolean settingDisplay) {
        float floatValue = (float) getBatteryLevel() / 1000;
        if (floatValue == 0) return " ";
        if (voltageDisplay || (settingDisplay && getBatteryDisplaySetting() == 0))
            return String.format("%.3f V", floatValue);
        else if (true) {
    ...
    String strVersionMBoard = "1.8";
    String[] strMBoardVersions = strVersionMBoard.split("\\.");
    int iBatteryNewCurveDelay;
    boolean bUsingInventoryBatteryCurve = false;
    ...
    int getBatteryValue2Percent(float floatValue) {
        boolean DEBUG = false;
        if (DEBUG)
            appendToLog("getHostProcessorICBoardVersion = " + getHostProcessorICBoardVersion() + ", strVersionMBoard = " + strVersionMBoard);
        if (false || checkHostProcessorVersion(getHostProcessorICBoardVersion(), Integer.parseInt(strMBoardVersions[0].trim()), Integer.parseInt(strMBoardVersions[1].trim()), 0)) {
            ...
            if (mRfidDevice.mRfidToWrite.size() != 0) iBatteryNewCurveDelay = 0;
                ...
                if (bUsingInventoryBatteryCurve == false) {
            ...
            if (DEBUG)
                appendToLog("NEW Percentage cureve is USED with bUsingInventoryBatteryCurve = " + bUsingInventoryBatteryCurve + ", iBatteryNewCurveDelay = " + iBatteryNewCurveDelay);
            ...
            else if (bUsingInventoryBatteryCurve && floatValue <= fBatteryValueOld && iValue >= iBatteryPercentOld)
                iValue = iBatteryPercentOld;
    ...
    public boolean getTriggerButtonStatus() {
        return mNotificationDevice.getTriggerStatus();
    }
    ...
    @Keep
    public Cs108ScanData getNewDeviceScanned() {
        if (mScanResultList.size() != 0) {
            appendToLog("mScanResultList.size() = " + mScanResultList.size());
            Cs108ScanData cs108ScanData = mScanResultList.get(0);
...
/*        if (false) return mScanResultList;
    ...
    public byte[] onBarcodeEvent() {
        ...
        if (barcodeCombined != null && mBarcodeDevice.getPrefix() != null && mBarcodeDevice.getSuffix() != null) {
            if (barcodeCombined.length == 0) barcodeCombined = null;
                ...
                byte[] prefixExpected = mBarcodeDevice.getPrefix();
                boolean prefixFound = false;
                byte[] suffixExpected = mBarcodeDevice.getSuffix();
    ...
    public boolean setRx000KillPassword(String password) {
        return mRfidDevice.mRx000Device.mRx000Setting.setRx000KillPassword(password);
    }

    @Keep
    public boolean setRx000AccessPassword(String password) {
        return mRfidDevice.mRx000Device.mRx000Setting.setRx000AccessPassword(password);
    }

    @Keep
    public boolean setAccessRetry(boolean accessVerfiy, int accessRetry) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessRetry(accessVerfiy, accessRetry);
    }

    @Keep
    public boolean setInvModeCompact(boolean invModeCompact) {
        return mRfidDevice.mRx000Device.mRx000Setting.setInvModeCompact(invModeCompact);
    }

    @Keep
    public boolean setAccessLockAction(int accessLockAction, int accessLockMask) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessLockAction(accessLockAction, accessLockMask);
    }

    @Keep
    public boolean setAccessBank(int accessBank) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessBank(accessBank);
    }

    @Keep
    public boolean setAccessBank(int accessBank, int accessBank2) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessBank(accessBank, accessBank2);
    }

    @Keep
    public boolean setAccessOffset(int accessOffset) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessOffset(accessOffset);
    }

    @Keep
    public boolean setAccessOffset(int accessOffset, int accessOffset2) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessOffset(accessOffset, accessOffset2);
    }

    @Keep
    public boolean setAccessCount(int accessCount) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessCount(accessCount);
    }

    @Keep
    public boolean setAccessCount(int accessCount, int accessCount2) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessCount(accessCount, accessCount2);
    }

    @Keep
    public boolean setAccessWriteData(String dataInput) {
        return mRfidDevice.mRx000Device.mRx000Setting.setAccessWriteData(dataInput);
    }

    @Keep
    public boolean setTagRead(int tagRead) {
        return mRfidDevice.mRx000Device.mRx000Setting.setTagRead(tagRead);
    }
    ...
    public String getSerialNumber() {
        return mRfidDevice.mRx000Device.mRx000OemSetting.getSerialNumber();
    }

    @Keep
    public boolean setInvBrandId(boolean invBrandId) {
        return mRfidDevice.mRx000Device.mRx000Setting.setInvBrandId(invBrandId);
    }

    @Keep
    public void macWrite(int address, int value) {
...
//            if (strActData.length() != 8)
//                appendToLogView("Warning: Invalid length of sensing data = " + strActData);
...
//                appendToLogView("Warning: Invalid checksum(" + String.valueOf(iChecksum) + ") for strActData = " + strActData);
        ...
        if (true || invalid == false) {
            ...
            if (iVersion == 0 || iVersion == 1)
                fTemperature = (float) (11984.47 / (21.25 + iD1 + iD2 / 2752 + fDelta2) - 301.57);
            else if (iVersion == 2) {
...
//            if (invalid) appendToLogView(String.format("Temperature = %f", fTemperature));
    ...
    float float16toFloat32(String strData) {
        ...
        if (strData.length() == 4) {
            ...
            if (DEBUG)
                appendToLog("strData = " + strData + ", iValue = " + iValue + ", iSign = " + iSign + ", iExp = " + iExp + ", iMant = " + iMant + ", fValue = " + fValue);
        }
        return fValue;
    }
    ...
    public String str2float16(String strData) {
        ...
        if (fValuePos < fValueMax) {
            ...
            if (DEBUG)
                appendToLog("bSign = " + bSign + ", iExp = " + iExp + ", iMant = " + iMant + ", iValue = " + iValue + ", strValue = " + strValue);
        }
        return strValue;
    }
