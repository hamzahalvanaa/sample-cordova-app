...
public class SaveList2ExternalTask extends AsyncTask<Void, Void, String> {
    ...
    protected String doInBackground(Void... params) {
        ...
        try {
            if (true) {
                ...
                if (validLocation == false)
                    resultDisplay += "Error in creating server location from " + mCs108Library4a.getServerLocation() + " !!!";
                else {
        ...
        if (errorDisplay != null)
            resultDisplay += "Error in sending data to server " + (url != null ? url : "") + " with " + errorDisplay;
    ...
    public JSONObject createJSON(ArrayList<ReaderDevice> tagsList0, ReaderDevice tagDevice0) {
        ...
        try {
            ...
            if (tagsList0 != null || tagDevice0 != null) {
                ...
                int i = 1;
                if (tagsList0 != null) i = tagsList0.size();
    ...
    String save2File(String messageStr, boolean requestPermission) {
        ...
        if (MainActivity.mCs108Library4a.getSaveFileEnable() == false)
    ...
    public void openServer() throws Exception {
        ...
        con.setConnectTimeout(MainActivity.mCs108Library4a.getServerTimeout() * 1000);
        errorDisplay = "Error in setRequestMethod()";
        con.setRequestMethod("POST");
        errorDisplay = "Error in setRequestProperty(User-Agent)";
        con.setRequestProperty("User-Agent", "Mozilla/5.0");
        errorDisplay = "Error in setRequestProperty(Accept-Languag)";
    ...
    public void write2Server(String messageStr0) {
        try {
            errorDisplay = "Error in getOutputStream()";
            OutputStream os = con.getOutputStream();
            errorDisplay = "Error in DataOutputStream()";
            DataOutputStream wr = new DataOutputStream(os);

            errorDisplay = "Error in writeBytes()";
            wr.writeBytes(messageStr0);

            errorDisplay = "Error in flush()";
            wr.flush();
            errorDisplay = "Error in close(wr)";
            wr.close();
            errorDisplay = "Error in close(os)";
            os.close();
            serverWritten = true;
        } catch (Exception ex) {
    ...
    public static class CustomPopupWindow {
        Context context;
        ...
        public void popupStart(String message, boolean wait) {
            ...
            int iLenghtMax = 300;
            if (message.length() > iLenghtMax)
                message = message.substring(0, iLenghtMax) + " .....";
