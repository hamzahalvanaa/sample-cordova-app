    ...
    public class LocationDevice {
        ...
        LocationDevice(Context context) {
            ...
            if (!(mPackageManager.hasSystemFeature(PackageManager.FEATURE_LOCATION))) {
                if (DEBUG)
                    mCs108Library4a.appendToLog("Cs108Connector(): there is NO Feature_Location");
        ...
        public void turnOn(boolean onStatus) {
            if (locationManager != null) {
                if (true)
                    mCs108Library4a.appendToLog("permission.ACCESS_FINE_LOCATION = " + ActivityCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_FINE_LOCATION));
                if (true)
                    mCs108Library4a.appendToLog("permission.ACCESS_COARSE_LOCATION = " + ActivityCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_COARSE_LOCATION));
                if (ActivityCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (true)
                        mCs108Library4a.appendToLog("LocationDevice.setRfidOn(): ON with LocationManager: ON");
                    if (true)
                        mCs108Library4a.appendToLog("LocationManager.PASSIVE_PROVIDER = " + LocationManager.PASSIVE_PROVIDER);
...
//                    locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 1000, 0, locationListener);
        ...
        private LocationListener locationListener = new LocationListener() {
            ...
            public void onLocationChanged(Location location) {
                if (true)
                    mCs108Library4a.appendToLog("onLocationChanged(): " + location.getProvider());
                location = location;
            }
...
*/
        ...
        SensorDevice(Context context) {
            ...
            if (DEBUG)
                mCs108Library4a.appendToLog("Cs108Connector.SensorDevice(): List of sensors");
            for (int i = 0; i < deviceSensors.size(); i++) {
                if (DEBUG)
                    mCs108Library4a.appendToLog(deviceSensors.get(i).getType() + "," + deviceSensors.get(i).getName());
            }
        }
        ...
        public void turnOn(boolean onStatus) {
            if (onStatus && this.onStatus == false) {
                ...
                if (mSensorManager.registerListener(sensorEventListener, mMagnetometer, SensorManager.SENSOR_DELAY_UI) == false) {
                    if (false)
                        Toast.makeText(mContext.getApplicationContext(), "MAGNETOMETER is NOT supported in this phone !!! Please use another phone.", Toast.LENGTH_LONG).show();
                    mMagnetometer = null;
                }
...
//                    appendToLog("onSensorChanged(): updated mAccelerometer");
                        ...
                        if (DEBUG)
                            mCs108Library4a.appendToLog("onSensorChanged(): mAccelerometer: " + event.values.length);
                    }
...
//                    appendToLog("onSensorChanged(): updated mMagnetometer");
                        ...
                        if (DEBUG)
                            mCs108Library4a.appendToLog("onSensorChanged(): mMagnetometer: " + event.values.length);
                    }
                } else {
                    if (DEBUG)
                        mCs108Library4a.appendToLog("onSensorChanged(): " + event.sensor.getType() + "," + event.sensor.getName());
                }
                ...
                if (mLastAccelerometerSet && mLastMagnetometerSet) {
                    ...
                    if (DEBUG)
                        mCs108Library4a.appendToLog("onSensorChanged(): updated mOrientation with mOrientation=" + mOrientation[0]);
                }
            }
