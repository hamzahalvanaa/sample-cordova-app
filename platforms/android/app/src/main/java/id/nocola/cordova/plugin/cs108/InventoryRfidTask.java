...
public class InventoryRfidTask extends AsyncTask<Void, String, String> {
    ...
    double rssi = 0;
    int phase, chidx, data1_count, data2_count, data1_offset, data2_offset;
    int port = -1;
    int portstatus;
    int backport1, backport2, codeSensor, codeRssi;
    float codeTempC;
    final int INVALID_CODEVALUE = -500;
    String brand;
    long timeMillis, startTimeMillis, runTimeMillis;
    long firstTime;
    long lastTime;
    boolean continousRequest = false;
    boolean finishingRequest = false;
    boolean debugEndRequest = false;
    ...
    protected void onPreExecute() {
        ...
        if (MainActivity.mCs108Library4a.getInventoryVibrate() && bUseVibrateMode0 == false && MainActivity.mCs108Library4a.getVibrateModeSetting() == 1 && MainActivity.mCs108Library4a.getAntennaDwell() == 0)
            bValidVibrateNewAll = true;
        ...
        while (MainActivity.mCs108Library4a.isBleConnected() && isCancelled() == false && ending == false) {
            ...
            if (rx000pkgData != null && MainActivity.mCs108Library4a.mrfidToWriteSize() == 0) {
                if (rx000pkgData.responseType == null) {
                    ...
                    if (rx000pkgData.decodedError != null)
                        endingMessaage = rx000pkgData.decodedError;
                    if (continousRequest) {
...
/*                if (System.currentTimeMillis() - timeMillisSound > 1000) {
            ...
            if (taskCancelReason != TaskCancelRReason.NULL) {
                ...
                if (finishingRequest) publishProgress("EEE");
...
//                    MainActivity.mCs108Library4a.appendToLogView("CANCELLING. sent abortOperation");
                if (output[0].contains("WW")) {
                    ...
                    if (timePeriod > 0) {
                        if (rfidRunTime != null)
                            rfidRunTime.setText(String.format("Run time: %d sec", timePeriod));
                        yieldRate = yield4RateCount;
                        yield4RateCount = 0;
                    }
                } else if (taskCancelReason == TaskCancelRReason.NULL) {
                    if (rfidVoltageLevel != null)
                        rfidVoltageLevel.setText(MainActivity.mCs108Library4a.getBatteryDisplay(true));
                }
            } else if (output[0].length() == 3) {
                mytoast = Toast.makeText(MainActivity.mContext, "Finishing tag data upload", Toast.LENGTH_LONG);
                mytoast.show();
            } else if (DEBUG)
                MainActivity.mCs108Library4a.appendToLog("InventoryRfidTask.InventoryRfidTask.onProgressUpdate(): " + output[0]);
        } else {
            long currentTime = 0;
            {
                while (rx000pkgDataArrary.size() != 0) {
                    ...
                    int extraLength = 0;
                    if (extra1Bank != -1 && rx000pkgData.decodedData1 != null)
                        extraLength += rx000pkgData.decodedData1.length;
                    if (extra2Bank != -1 && rx000pkgData.decodedData2 != null)
                        extraLength += rx000pkgData.decodedData2.length;
                    if (extraLength != 0) {
                    ...
                    codeTempC = INVALID_CODEVALUE;
                    brand = null;
                    String strExtra2 = null;
                    if (rx000pkgData.decodedData2 != null)
                        strExtra2 = MainActivity.mCs108Library4a.byteArrayToString(rx000pkgData.decodedData2);
                    ...
                    String strCrc16 = null;
                    if (rx000pkgData.decodedCrc != null)
                        strCrc16 = MainActivity.mCs108Library4a.byteArrayToString(rx000pkgData.decodedCrc);
                    ...
                    boolean bFastId = false;
                    boolean bTempId = false;
                    ...
                    String strXpc = null;
                    int iSensorData = ReaderDevice.INVALID_SENSORDATA;
                    if ((iPc & 0x0200) != 0) {
                    ...
                    if (match == false) {
                        if (tagsList == null) {
                            ...
                            SharedObjects.TagsIndex tagsIndex = new SharedObjects.TagsIndex(strAddresss, tagsList.size() - 1);
...
//                                    saveExternalTask = new SaveList2ExternalTask();
//                                    saveExternalTask.openServer();
                                    String msgOutput = saveExternalTask.createJSON(null, readerDevice).toString();
                                    MainActivity.mCs108Library4a.appendToLog("Json = " + msgOutput);
            ...
            if (invalidDisplay) {
                if (rfidYieldView != null)
                    rfidYieldView.setText(String.valueOf(total) + "," + String.valueOf(MainActivity.mCs108Library4a.validata));
                if (rfidRateView != null)
                    rfidRateView.setText(String.valueOf(MainActivity.mCs108Library4a.invalidata) + "," + String.valueOf(MainActivity.mCs108Library4a.mRfidDevice.mRx000Device.invalidUpdata));
            } else {
                ...
                if (total != 0) {
                    ...
                    if (lastTime == 0) {
                        ...
                        strRate += "Rate:" + String.valueOf(yieldRate) + "/" + String.valueOf(totalOld * 1000 / (currentTime - firstTimeOld));
...
//            if (false)
//                MainActivity.mCs108Library4a.appendToLogView("playerN = " + (playerN == null ? "Null" : "Valid") + ", playerO = " + (playerO == null ? "Null" : "Valid"));
            if (playerN != null && playerO != null) {
//                if (false)
//                    MainActivity.mCs108Library4a.appendToLogView("requestSound = " + requestSound + ", bStartBeepWaiting = " + bStartBeepWaiting + ", Op=" + playerO.isPlaying() + ", Np=" + playerN.isPlaying());
                if (requestSound && playerO.isPlaying() == false && playerN.isPlaying() == false) {
                    if (true) {
                        if (bStartBeepWaiting == false) {
                            bStartBeepWaiting = true;
//                            if (false)
//                                MainActivity.mCs108Library4a.appendToLogView("Going to play old song");
                        ...
                        if (MainActivity.mCs108Library4a.getInventoryVibrate()) {
                            ...
                            if (validVibrate0) {
                                if (bStartVibrateWaiting == false) {
                                    ...
                                    handler.removeCallbacks(runnableStartVibrate);
                                    int timeout = MainActivity.mCs108Library4a.getVibrateWindow() * 1000;
...
//            if (false) MainActivity.mCs108Library4a.appendToLogView("Playing old song");
    ...
    protected void onCancelled() {
        super.onCancelled();
        if (DEBUG)
            MainActivity.mCs108Library4a.appendToLog("InventoryRfidTask.InventoryRfidTask.onCancelled()");
    ...
    protected void onPostExecute(String result) {
        if (DEBUG)
            MainActivity.mCs108Library4a.appendToLog("InventoryRfidTask.InventoryRfidTask.onPostExecute(): " + result);
    ...
    public InventoryRfidTask(Context context, int extra1Bank, int extra2Bank, int data1_count, int data2_count, int data1_offset, int data2_offset,
        ...
        if (DEBUG)
            MainActivity.mCs108Library4a.appendToLog("data1_count = " + data1_count + ", data2_count = " + data2_count + ", extra1Bank = " + extra1Bank + ", extra2Bank = " + extra2Bank);
        ...
        MainActivity.mCs108Library4a.appendToLog("HelloK: strMdid = " + strMdid);
...
//            MainActivity.mCs108Library4a.appendToLogView("playerO and playerN is created");
    ...
    void DeviceConnectTask4InventoryEnding(TaskCancelRReason taskCancelRReason) {
        ...
        if (endingRequest) {
            switch (taskCancelReason) {
                case NULL:
                    mytoast = Toast.makeText(MainActivity.mContext, "Finish as COMMAND END is received", Toast.LENGTH_SHORT);
                    break;
                case STOP:
                    mytoast = Toast.makeText(MainActivity.mContext, "Finish as STOP is pressed", Toast.LENGTH_SHORT);
                    break;
                case BUTTON_RELEASE:
                    mytoast = Toast.makeText(MainActivity.mContext, "Finish as BUTTON is released", Toast.LENGTH_SHORT);
                    break;
                case TIMEOUT:
                    mytoast = Toast.makeText(MainActivity.mContext, "Finish as TIMEOUT", Toast.LENGTH_SHORT);
                    break;
                case RFID_RESET:
                    mytoast = Toast.makeText(MainActivity.mContext, "Finish as RFID RESET", Toast.LENGTH_SHORT);
                    break;
                case INVALD_REQUEST:
                    mytoast = Toast.makeText(MainActivity.mContext, "Invalid sendHostRequest. Operation is cancelled.", Toast.LENGTH_SHORT);
                    break;
            }
    ...
    String decodeMicronData(String strActData, String strCalData) {
        ...
        if (iTag35 == 5) {
            backport1 = Integer.parseInt(strActData.substring(0, 4), 16);
            backport2 = Integer.parseInt(strActData.substring(4, 8), 16);
            MainActivity.mCs108Library4a.appendToLog("backport1 = " + backport1 + ", backport2 = " + backport2);
            strActData = strActData.substring(8);
        }
        int iSensorCode = Integer.parseInt(strActData.substring(0, 4), 16);
        iSensorCode &= 0x1FF;
        if (iTag35 == 2) iSensorCode &= 0x1F;
        codeSensor = iSensorCode;
        int iRssi;
        String strRetValue = "";
        if (iTag35 == 2) {
            iRssi = Integer.parseInt(strCalData.substring(0, 4), 16);
            iRssi &= 0x1F;
            codeRssi = iRssi;
        } else if (iTag35 == 3) {
            iRssi = Integer.parseInt(strActData.substring(4, 8), 16);
            iRssi &= 0x1F;
            codeRssi = iRssi;
