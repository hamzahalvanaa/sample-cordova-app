...
public class Cs108Connector extends BleConnector {
    ...
    void disconnect() {
        ...
        mRfidDevice.mRx000Device.mRx000ToWrite.clear();
    }
    ...
    boolean writeData(byte[] buffer, int timeout) {
        ...
        if (DEBUG) appendToLog("result = " + result + ", characteristicListRead = " + characteristicListRead + ", btSendTimeOut = " + btSendTimeOut);
        return result;
    }
...
//        else mLogView.append(System.currentTimeMillis() - ltimeLogView + ": " + s + "\n" );
//    }
...
//            if (streamInMissing != 0) appendToLogView("processCs108DataIn(" + getStreamInTotalCounter() + ", " + getStreamInAddCounter() + "): len=0, getStreamInOverflowTime()=" + streamInOverflowTime + ", MissBytes=" + streamInMissing + ", Offset=" + cs108DataLeftOffset);
            ...
            if (len != 0 && bFirst) { bFirst = false; writeDebug2File("B" + String.valueOf(intervalProcessCs108Data) + ", " + System.currentTimeMillis()); }
...
//                    if (false) appendToLogView("ERROR, ENDLOOP invalid data content: " + invalidPart.length + ", " + byteArrayToString(invalidPart) + ", with start=" + cs108DataReadStart + ", new offset=" + cs108DataLeftOffset);
    ...
    Context context;
    Cs108Connector(Context context) {
        super(context);
...
//                        appendToLogView(". Timeout !!! mCs108DataRead.size() = " + mCs108DataRead.size());
            ...
            if (ready2Write) {
                ...
                timer2Write = 0;
                if (mRfidDevice.mRx000Device.mRx000ToWrite.size() != 0 && mRfidDevice.mRfidToWrite.size() == 0) {
...
//                            appendToLogView("failure to send " + mBarcodeToWrite.get(0).barcodePayloadEvent.toString());
        ...
        private boolean isBarcodeToRead(Cs108ReadData cs108ReadData) {
            ...
            if (cs108ReadData.dataValues[0] == (byte) 0x91) {
                if (DEBUG) appendToLog("dataValues = " + byteArrayToString(cs108ReadData.dataValues));
                            ...
                            if (mBarcodeToWrite.get(0).downlinkResponsed) {
                                ...
                                if (mBarcodeToWrite.get(0).dataValues[0] == 0x1b) {
                                    ...
                                    if (dataValues.length >= 5 + 1) {
                                        if (dataValues[0] == 2 && dataValues[1] == 0 && dataValues[4] == 0x34) {
                                            int length = dataValues[2] * 256 + dataValues[3];
                                            if (dataValues.length >= length + 5) {
                                                if (mBarcodeToWrite.get(0).dataValues[5] == 0x37 && length >= 5) {
                                                    matched = true;
                                                    int prefixLength = dataValues[6];
                                                    int suffixLength = 0;
                                                    if (dataValues.length >= 5 + 5 + prefixLength) {
                                                        suffixLength = dataValues[6 + prefixLength + 2];
                                                    }
                                                    if (dataValues.length >= 5 + 5 + prefixLength + suffixLength) {
                                                        bytesBarcodePrefix = null;
                                                        bytesBarcodeSuffix = null;
                                                        if (dataValues[5] == 1) {
                                                            bytesBarcodePrefix = new byte[prefixLength];
                                                            System.arraycopy(dataValues, 7, bytesBarcodePrefix, 0, bytesBarcodePrefix.length);
                                                        }
                                                        if (dataValues[6 + prefixLength + 1] == 1) {
                                                    ...
                                                    if (dataValues[5] == mBarcodeToWrite.get(0).dataValues[6] && dataValues[6] == mBarcodeToWrite.get(0).dataValues[7]) {
                                                        ...
                                                        System.arraycopy(dataValues, 7, byteSN, 0, byteSN.length);
                                                        try {
        ...
        private boolean arrayTypeSet(byte[] dataBuf, int pos, NotificationPayloadEvents event) {
            ...
            switch (event) {
                ...
                case NOTIFICATION_AUTO_RFIDINV_ABORT:
                    if (checkHostProcessorVersion(mBluetoothIcDevice.getBluetoothIcVersion(), 1, 0, 13) == false) {
                        validEvent = false;
                    } else {
                        dataBuf[pos] = 4;
                        validEvent = true;
                ...
                case NOTIFICATION_AUTO_BARINV_STARTSTOP:
                    if (checkHostProcessorVersion(mBluetoothIcDevice.getBluetoothIcVersion(), 1, 0, 14) == false) {
                        validEvent = false;
                    } else {
                        dataBuf[pos] = 6;
                        validEvent = true;
                ...
                case NOTIFICATION_GET_AUTO_BARINV_STARTSTOP:
                    if (false && checkHostProcessorVersion(mBluetoothIcDevice.getBluetoothIcVersion(), 1, 0, 14) == false) {
                        validEvent = false;
                    } else {
                        dataBuf[pos] = 7;
                        validEvent = true;
...
//                            appendToLogView("failure to send " + mSiliconLabIcToWrite.get(0).toString());
    ...
    class BluetoothIcDevice {
        ...
        private ArrayList<Cs108BluetoothIcData> mBluetoothIcToWrite = new ArrayList<>();
        ...
        private boolean writeBluetoothIc(Cs108BluetoothIcData data) {
            int datalength = 0;
        ...
        private boolean isMatchBluetoothIcToWrite(Cs108ReadData cs108ReadData) {
            boolean match = false;
            if (mBluetoothIcToWrite.size() != 0) {
        ...
        private int sendDataToWriteSent = 0;
        private boolean sendBluetoothIcToWrite() {
            if (mBluetoothIcToWrite.size() != 0) {
                if (isBleConnected() == false) {
                    mBluetoothIcToWrite.clear();
                } else if (System.currentTimeMillis() - btSendTime > btSendTimeOut) {
                    if (sendDataToWriteSent >= 5) {
                        ...
                        if (DEBUG) appendToLog("Removed after sending count-out.");
                        Toast.makeText(context, "Problem in sending data to Bluetooth Module. Removed data sending after count-out", Toast.LENGTH_SHORT).show();
                    } else {
                        ...
                        boolean retValue = writeBluetoothIc(mBluetoothIcToWrite.get(0));
                        if (retValue) {
                            sendDataToWriteSent++;
                        } else {
//                            appendToLogView("failure to send " + mBluetoothIcToWrite.get(0).bluetoothIcPayloadEvent.toString());
    ...
    class Rx000Setting {
        ...
        boolean readMAC(int address) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.MAC_OPERATION, false, msgBuffer);
        }

        boolean writeMAC(int address, int value) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.MAC_OPERATION, true, msgBuffer);
        }
        ...
        String getMacVer() {
            if (macVer == null) {
                if (true)   readMAC(0);
                //byte[] msgBuffer = new byte[]{(byte) 0x70, 0, 0, 0, 0, 0, 0, 0};
                //mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.MAC_VER, false, msgBuffer);
                return "";
            } else {
                return macVer;
            }
...
//            if (bCommmandActive) msgBuffer[5] |= 0x20;
        ...
        boolean setAntennaCycle(int antennaCycle, int antennaFreqAgile) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_CYCLES, true, msgBuffer);
        }
        ...
        private boolean getHST_ANT_CYCLES() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_CYCLES, false, msgBuffer);
        }
        ...
        int getAntennaSelect() {
            if (antennaSelect < ANTSLECT_MIN || antennaSelect > ANTSELECT_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_SEL, false, msgBuffer);
            }
        ...
        boolean setAntennaSelect(int antennaSelect) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_SEL, true, msgBuffer);
        }
...
//                return antennaSelectedData[antennaSelect].getAntennaPower();
        ...
        int getInvSelectIndex() {
            if (invSelectIndex < INVSELECT_MIN || invSelectIndex > INVSELECT_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGMSK_DESC_SEL, false, msgBuffer);
            }
        ...
        boolean setInvSelectIndex(int invSelect) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGMSK_DESC_SEL, true, msgBuffer);
        }
        ...
        int getSelectMaskBank() {
            ...
            if (invSelectIndex < INVSELECT_MIN || invSelectIndex > INVSELECT_MAX) invSelectIndex = mDefault.invSelectIndex;
            return invSelectData[dataIndex].getSelectMaskBank();
        }
        ...
        boolean setQueryTarget(int queryTarget, int querySession, int querySelect) {
            if (queryTarget >= 2) { mRfidDevice.mRx000Device.mRx000Setting.setAlgoAbFlip(1); }
            else if (queryTarget >= 0) { mRfidDevice.mRx000Device.mRx000Setting.setAlgoAbFlip(0); }
        ...
        final int QUERYSESSION_INVALID = 0; final int QUERYSESSION_MIN = 0; final int QUERYSESSION_MAX = 3;
        ...
        private boolean getHST_QUERY_CFG() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_QUERY_CFG, false, msgBuffer);
        }
        ...
        boolean setCycleDelay(long cycleDelay) {
            ...
            boolean bResult = mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_CFG, true, msgBuffer);
            msgBuffer = new byte[]{(byte) 0x70, 0, (byte)0x0F, (byte)0x0F, 0, 0, 0, 0};
            mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_CFG, false, msgBuffer);
            return bResult;
        }
        ...
        private boolean getHST_AUTHENTICATE_CFG() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_AUTHENTICATE_CFG, false, msgBuffer);
        }
        boolean setHST_AUTHENTICATE_CFG(boolean sendReply, boolean incReplyLenth, int length) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_AUTHENTICATE_CFG, true, msgBuffer);
        }
            ...
            for (int i = 0; i < 3; i++) {
                if (length > 0) {
                    ...
                    if ((authMatchDataReady & (0x01 << i)) == 0) {
                        ...
                        msgBuffer[2] += i;
                        mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_AUTHENTICATE_MSG, false, msgBuffer);
                    } else {
                        for (int j = 0; j < 4; j++) {
        ...
        boolean setAuthMatchData(String matchData) {
            ...
            for (int i = 0; i < 6; i++) {
                if (length > 0) {
                    ...
                    if (mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_AUTHENTICATE_MSG, true, msgBuffer) == false)
                        return false;
                    else {
...
//                        appendToLog("invMatchDataReady=" + Integer.toString(mRx000Setting.invMatchDataReady, 16) + ", message=" + byteArrayToString(msgBuffer));
        ...
        private boolean getHST_UNTRACEABLE_CFG() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_UNTRACEABLE_CFG, false, msgBuffer);
        }
        boolean setHST_UNTRACEABLE_CFG(int range, boolean user, int tid, int epcLength, boolean epc, boolean uxpc) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_UNTRACEABLE_CFG, true, msgBuffer);
        }
        ...
        boolean setInvAlgo(int invAlgo, int matchRep, int tagSelect, int noInventory, int tagRead, int tagDelay, int invModeCompact, int invBrandId) {
            ...
            if (DEBUG) appendToLog("Old invAlgo = " + this.invAlgo + ", matchRep = " + this.matchRep + ", tagSelect =" + this.tagSelect + ", noInventory = " + this.noInventory + ", tagRead = " + this.tagRead + ", tagDelay = " + this.tagDelay + ", invModeCompact = " + this.invModeCompact);
            if (DEBUG) appendToLog("New invAlgo = " + invAlgo + ", matchRep = " + matchRep + ", tagSelect =" + tagSelect + ", noInventory = " + noInventory + ", tagRead = " + tagRead + ", tagDelay = " + tagDelay + ", invModeCompact = " + invModeCompact + ", sameCheck = " + sameCheck);
            if (this.invAlgo == invAlgo && this.matchRep == matchRep && this.tagSelect == tagSelect && this.noInventory == noInventory && this.tagRead == tagRead && this.tagDelay == tagDelay && this.invModeCompact == invModeCompact && sameCheck) return true;
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_CFG, true, msgBuffer);
        }
        ...
        int getAlgoSelect() {
            if (algoSelect < ALGOSELECT_MIN || algoSelect > ALGOSELECT_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_SEL, false, msgBuffer);
            }
            return algoSelect;
        }
        boolean dummyAlgoSelected = false;
        boolean setAlgoSelect(int algoSelect) {
            appendToLog("algoSelect = " + algoSelect + ", this.algoSelct = " + this.algoSelect + ", dummyAlgoSelected = " + dummyAlgoSelected);
        ...
        private boolean getHST_INV_EPC_MATCH_CFG() {
            if (matchEnable < MATCHENABLE_MIN || matchEnable > MATCHENABLE_MAX
                ...
                return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_EPC_MATCH_CFG, false, msgBuffer);
            } else {
                return false;
            }
        }
        private boolean setHST_INV_EPC_MATCH_CFG(int matchEnable, int matchType, int matchLength, int matchOffset) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_EPC_MATCH_CFG, true, msgBuffer);
        }
            ...
            for (int i = 0; i < 16; i++) {
                if (length > 0) {
                    if ((invMatchDataReady & (0x01 << i)) == 0) {
                        ...
                        msgBuffer[2] += i;
                        mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_EPCDAT_0_3, false, msgBuffer);
        ...
        boolean setInvMatchData(String matchData) {
            ...
            for (int i = 0; i < 16; i++) {
                if (length > 0) {
                    ...
                    msgBuffer[2] = (byte) ((msgBuffer[2] & 0xFF) + i);
                    if (mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_EPCDAT_0_3, true, msgBuffer) == false)
                        return false;
                    else {
...
//                        appendToLog("invMatchDataReady=" + Integer.toString(mRx000Setting.invMatchDataReady, 16) + ", message=" + byteArrayToString(msgBuffer));
        ...
        int getAccessRetry() {
            if (accessRetry < ACCRETRY_MIN || accessRetry > ACCRETRY_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_DESC_CFG, false, msgBuffer);
            }
            return accessRetry;
        }
        boolean setAccessRetry(boolean accessVerfiy, int accessRetry) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_DESC_CFG, true, msgBuffer);
        }
        ...
        int getAccessBank() {
            if (accessBank < ACCBANK_MIN || accessBank > ACCBANK_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_BANK, false, msgBuffer);
            }
            return accessBank;
        }
        boolean setAccessBank(int accessBank) {
            ...
            if (this.accessBank == accessBank && sameCheck) return true;
            msgBuffer[4] = (byte) (accessBank & 0x03);
            this.accessBank = accessBank;
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_BANK, true, msgBuffer);
        }
        ...
        int getAccessOffset() {
            if (accessOffset < ACCOFFSET_MIN || accessOffset > ACCOFFSET_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_PTR, false, msgBuffer);
            }
            return accessOffset;
        }
        boolean setAccessOffset(int accessOffset) {
            ...
            if (this.accessOffset == accessOffset && sameCheck) return true;
            msgBuffer[4] = (byte) (accessOffset & 0xFF);
            msgBuffer[5] = (byte) ((accessOffset >> 8) & 0xFF);
            this.accessOffset = accessOffset;
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_PTR, true, msgBuffer);
        }
        ...
        int getAccessCount() {
            if (accessCount < ACCCOUNT_MIN || accessCount > ACCCOUNT_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_CNT, false, msgBuffer);
            }
            return accessCount;
        }
        boolean setAccessCount(int accessCount) {
            ...
            if (this.accessCount == accessCount && sameCheck) return true;
            msgBuffer[4] = (byte) (accessCount & 0xFF);
            this.accessCount = accessCount;
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_CNT, true, msgBuffer);
        }
        ...
        boolean getHST_TAGACC_LOCKCFG() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_LOCKCFG, false, msgBuffer);
        }
        boolean setAccessLockAction(int accessLockAction, int accessLockMask) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_LOCKCFG, true, msgBuffer);
        }
        ...
        boolean setRx000AccessPassword(String password) {
            ...
            boolean retValue = mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_ACCPWD, true, msgBuffer);
        ...
        boolean setRx000KillPassword(String password) {
            ...
            boolean retValue = mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGACC_KILLPWD, true, msgBuffer);
            if (DEBUG) appendToLog("sendHostRegRequest(): retValue = " + retValue);
            return retValue;
        }
        ...
        int getAccessWriteDataSelect() {
            if (accessWriteDataSelect < ACCWRITEDATSEL_MIN || accessWriteDataSelect > ACCWRITEDATSEL_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGWRDAT_SEL, false, msgBuffer);
            }
        ...
        boolean setAccessWriteDataSelect(int accessWriteDataSelect) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGWRDAT_SEL, true, msgBuffer);
        }
            ...
            for (int i = 0; i < 32; i++) {
                if (length > 0) {
                    if ((accWriteDataReady & (0x01 << i)) == 0) {
                        ...
                        msgBuffer[2] += i;
                        mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGWRDAT_0, false, msgBuffer);
            ...
            for (int i = 0; i < writeBufLength; i++) {
                if (length > 0) {
                    ...
                    if ((i / 16) * 16 == i) {
                        ...
                        if (mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGWRDAT_SEL, true, msgBuffer) == false) {
                            appendToLog("23");
                            return false;
                        }
                    }
...
//                        }
                    ...
                    if (mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGWRDAT_0, true, msgBuffer) == false) {
                        appendToLog("3"); return false;
                    } else {
                        mRfidDevice.mRx000Device.mRx000Setting.accWriteDataReady |= (0x01 << i);
        ...
        int getCurrentProfile() {
            if (currentProfile < PROFILE_MIN || currentProfile > PROFILE_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_CURRENT_PROFILE, false, msgBuffer);
            }
            return currentProfile;
        }
        boolean setCurrentProfile(int currentProfile) {
            byte[] msgBuffer = new byte[]{(byte) 0x70, 1, 0x60, 0x0B, 0, 0, 0, 0};
            if (currentProfile < PROFILE_MIN || currentProfile > PROFILE_MAX)   currentProfile = mDefault.currentProfile;
            if (this.currentProfile == currentProfile && sameCheck)  return true;
            msgBuffer[4] = (byte) (currentProfile);
            this.currentProfile = currentProfile;
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_CURRENT_PROFILE, true, msgBuffer);
        }
        ...
        int getFreqChannelSelect() {
            if (freqChannelSelect < FREQCHANSEL_MIN || freqChannelSelect > FREQCHANSEL_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_SEL, false, msgBuffer);
            }
        ...
        boolean setFreqChannelSelect(int freqChannelSelect) {
            ...
            appendToLog("Frequency0: freqChannelSelect = " + freqChannelSelect + ", this.freqChannelSelect = " + this.freqChannelSelect);
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_SEL, true, msgBuffer);
        }
        ...
        int getFreqChannelConfig() {
            if (freqChannelConfig < FREQCHANCONFIG_MIN || freqChannelConfig > FREQCHANCONFIG_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_CFG, false, msgBuffer);
            }
...
//            if (onCurrent == on && sameCheck)  return true;
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_CFG, true, msgBuffer);
        }
        ...
        int getFreqPllMultiplier() {
            if (freqPllMultiplier == FREQPLLMULTIPLIER_INVALID) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_DESC_PLLDIVMULT, false, msgBuffer);
            }
        ...
        boolean setFreqPllMultiplier(int freqPllMultiplier) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_DESC_PLLDIVMULT, true, msgBuffer);
        }
        ...
        int getFreqPllDac() {
            if (freqPllDac == FREQPLLDAC_INVALID) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_DESC_PLLDACCTL, false, msgBuffer);
            }
            return freqPllDac;
        }
        ...
        boolean setFreqChannelOverride(int freqStart) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_RFTC_FRQCH_CMDSTART, true, msgBuffer);
        }
    }
    ...
    class AntennaSelectedData {
        ...
        private boolean getHST_ANT_DESC_CFG() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_CFG, false, msgBuffer);
        }
        boolean setAntennaEnable(int antennaEnable, int antennaInventoryMode, int antennaLocalAlgo, int antennaLocalStartQ,
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_CFG, true, msgBuffer);
        }
        ...
        int getAntennaStatus() {
            if (antennaStatus < ANTSTATUS_MIN || antennaStatus > ANTSTATUS_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.MAC_ANT_DESC_STAT, false, msgBuffer);
            }
        ...
        int getAntennaDefine() {
            if (antennaDefine < ANTDEFINE_MIN || antennaDefine > ANTDEFINE_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_PORTDEF, false, msgBuffer);
            }
        ...
        long getAntennaDwell() {
            if (antennaDwell < ANTDWELL_MIN || antennaDwell > ANTDWELL_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_DWELL, false, msgBuffer);
            }
            return antennaDwell;
        }
        boolean setAntennaDwell(long antennaDwell) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_DWELL, true, msgBuffer);
        }
        ...
        long getAntennaPower() {
            if (antennaPower < ANTPOWER_MIN || antennaPower > ANTPOWER_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_RFPOWER, false, msgBuffer);
            }
            return antennaPower;
        }
        boolean setAntennaPower(long antennaPower) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_RFPOWER, true, msgBuffer);
        }
        ...
        long getAntennaInvCount() {
            if (antennaInvCount < ANTINVCOUNT_MIN || antennaInvCount > ANTINVCOUNT_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_INV_CNT, false, msgBuffer);
            }
        ...
        boolean setAntennaInvCount(long antennaInvCount) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_ANT_DESC_INV_CNT, true, msgBuffer);
        }
    }
    ...
    class InvSelectData {
        ...
        int getSelectMaskBank() {
            if (selectMaskBank < INVSELMBANK_MIN || selectMaskBank > INVSELMBANK_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGMSK_BANK, false, msgBuffer);
            }
        ...
        int getSelectMaskOffset() {
            if (selectMaskOffset < INVSELMOFFSET_MIN || selectMaskOffset > INVSELMOFFSET_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGMSK_PTR, false, msgBuffer);
            }
        ...
        int getSelectMaskLength() {
            if (selectMaskLength < INVSELMLENGTH_MIN || selectMaskOffset > INVSELMLENGTH_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGMSK_LEN, false, msgBuffer);
            }
                ...
                for (int i = 0; i < 8; i++) {
                    if (length > 0) {
                        if ((selectMaskDataReady & (0x01 << i)) == 0) {
                            ...
                            msgBuffer[2] += i;
                            mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_TAGMSK_0_3, false, msgBuffer);
...
//                                appendToLog("setSelectMaskData(" + maskData +"): i=" + i + ", j=" + j + ", k=" + k);
    ...
    class AlgoSelectedData {
        ...
        private boolean getHST_INV_ALG_PARM_0() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_ALG_PARM_0, false, msgBuffer);
        }
        boolean setAlgoStartQ(int startQ, int algoMaxQ, int algoMinQ, int algoMaxRep, int algoHighThres, int algoLowThres) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_ALG_PARM_0, true, msgBuffer);
        }
        ...
        int getAlgoRetry() {
            if (algoRetry < ALGORETRY_MIN || algoRetry > ALGORETRY_MAX) {
                ...
                mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_ALG_PARM_1, false, msgBuffer);
            }
            return algoRetry;
        }
        boolean setAlgoRetry(int algoRetry) {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_ALG_PARM_1, true, msgBuffer);
        }
        ...
        private boolean getHST_INV_ALG_PARM_2() {
            ...
            return mRfidDevice.mRx000Device.sendHostRegRequest(HostRegRequests.HST_INV_ALG_PARM_2, false, msgBuffer);
        }
        boolean setAlgoAbFlip(int algoAbFlip, int algoRunTilZero) {
    ...
    public class Rx000Device {
        ...
        private void clearTempDataIn() {
            clearTempDataIn_request = false;
            streamInBufferReseting = true;
//            dataInBufferResetting = true;
            //mRfidReadingReset = true;

//            byte[] tempBuf = new byte[100]; while (readData(tempBuf, 0, tempBuf.length) != 0) { }
            cs108DataLeft = new byte[CS108DATALEFT_SIZE];
            cs108DataLeftOffset = 0;
            mCs108DataRead.clear();
...
//                    appendToLogView(". Timeout !!! mRfidToRead.size() = " + mRfidDevice.mRfidToRead.size());
                    ...
                    while (mRfidToReadingOffset > startIndex) {
                        ...
                        if (mRfidToReadingOffset - startIndex >= 8) {
                            if (mRfidToReading[startIndex + 0] == (byte) 0x40
                                ...
                                if (mRfidDevice.mRfidToWrite.size() == 0) {
                                    ...
                                    mRfidDevice.mRfidToWrite.remove(0); if (DEBUG) appendToLog("Control Response is received with null mRfidToWrite.dataValues");
                                    ...
                                    if (DEBUG) appendToLog("Control Response is received with Mis-matched mRfidToWrite, " + startIndex + ", " + byteArrayToString(dataIn));
                                } else {
                                    ...
                                    if (!(compareArray(dataInCompare, dataIn8, 8))) {
                                        if (DEBUG) appendToLog("Control response with invalid data: " + byteArrayToString(dataIn8));
                                    } else {
                        ...
                        /*else if (dataIn[startIndex + 1] == 3 && mRx000ToWrite.size() != 0) {   //input as GetVersion Control Command Response
                            if (!( mRx000ToWrite.get(0).dataValues[0] == (byte)0xC0 && mRx000ToWrite.get(0).dataValues[1] == 6)) {
                                appendToLog("mRx000UplinkHandler(): GertVersion Control Response is received with Mis-matched mRx000ToWrite");
                            } else if (dataIn.length - startIndex >= dataIn[startIndex + 0]) {
                                byte[] byteVersion = new byte[dataIn.length - 2];
                                System.arraycopy(dataIn, 2, byteVersion, 0, byteVersion.length);
                                try {
                                    strVersion = new String(byteVersion, "UTF-16LE");
                                } catch (Exception ex) {
                                    //
                                }
                                mRx000ToWrite.remove(0);
                                startIndex += dataIn[startIndex + 0];
                                appendToLog("mRx000UplinkHandler(): Version = " + strVersion + "with mRx000ToWrite.size=" + mRx000ToWrite.size());
                            }
                        }*/
                                    ...
                                    if (addressToRead != addressToWrite) {
                                        ...
                                        switch (addressToRead) {
                                            ...
                                            case 0x0201:
                                                ...
                                                if (true) appendToLog("found diagnostic configuration: " + byteArrayToString(dataInPayload) + ", diagnosticCfg=" + mRx000Setting.diagnosticCfg);
                                                break;
                                            case 0x0203:
                                            ...
                                            case 0x0700:
                                                ...
                                                if (DEBUG) appendToLog("found antenna cycle: " + byteArrayToString(dataInPayload) + ", cycle=" + mRx000Setting.antennaCycle + ", frequencyAgile=" + mRx000Setting.antennaFreqAgile);
                                                break;
                                            case 0x0701:
                                            ...
                                            case 0x0702:
                                                ...
                                                if (DEBUG) appendToLog("found antenna selectEnable: " + byteArrayToString(dataInPayload)
                                            ...
                                            case 0x0703:
                                                ...
                                                if (DEBUG) appendToLog("found antenna status: " + byteArrayToString(dataInPayload) + ", status=" + mRx000Setting.antennaSelectedData[mRx000Setting.antennaSelect].antennaStatus);
                                                break;
                                            case 0x0704:
                                                ...
                                                if (DEBUG) appendToLog("found antenna define: " + byteArrayToString(dataInPayload)
                                            ...
                                            case 0x0706:
                                                ...
                                                if (DEBUG) appendToLog("found antenna power=" + mRx000Setting.antennaSelectedData[mRx000Setting.antennaSelect].antennaPower);
                                                break;
                                            case 0x0707:
                                            ...
                                            case 0x0800:
                                                ...
                                                if (DEBUG) appendToLog("found inventory select: " + byteArrayToString(dataInPayload) + ", select=" + mRx000Setting.invSelectIndex);
                                                break;
                                            case 0x0801: {
                                                int dataIndex = mRx000Setting.invSelectIndex;
                                                if (dataIndex < mRx000Setting.INVSELECT_MIN || dataIndex > mRx000Setting.INVSELECT_MAX) {
                                                    if (DEBUG) appendToLog("found inventory select configuration: " + byteArrayToString(dataInPayload) + ", but invalid index=" + dataIndex);
                                                } else {
                                            ...
                                            case 0x0802: {
                                                int dataIndex = mRx000Setting.invSelectIndex;
                                                if (dataIndex < mRx000Setting.INVSELECT_MIN || dataIndex > mRx000Setting.INVSELECT_MAX) {
                                                    if (DEBUG) appendToLog("found inventory select mask bank: " + byteArrayToString(dataInPayload) + ", but invalid index=" + dataIndex);
                                                } else {
                                                    mRx000Setting.invSelectData[dataIndex].selectMaskBank = (dataIn[startIndex + 4] & 0x03);
                                                    if (DEBUG) appendToLog("found inventory select mask bank: " + byteArrayToString(dataInPayload)
                                            ...
                                            case 0x0803: {
                                                int dataIndex = mRx000Setting.invSelectIndex;
                                                if (dataIndex < mRx000Setting.INVSELECT_MIN || dataIndex > mRx000Setting.INVSELECT_MAX) {
                                                    if (DEBUG) appendToLog("found inventory select mask offset: " + byteArrayToString(dataInPayload) + ", but invalid index=" + dataIndex);
                                                } else {
                                                    ...
                                                    if (DEBUG) appendToLog("found inventory select mask offset: " + byteArrayToString(dataInPayload)
                                            ...
                                            case 0x0804: {
                                                int dataIndex = mRx000Setting.invSelectIndex;
                                                if (dataIndex < mRx000Setting.INVSELECT_MIN || dataIndex > mRx000Setting.INVSELECT_MAX) {
                                                    if (DEBUG) appendToLog("found inventory select mask length: " + byteArrayToString(dataInPayload) + ", but invalid index=" + dataIndex);
                                                } else {
                                                    mRx000Setting.invSelectData[dataIndex].selectMaskLength = (dataIn[startIndex + 4] & 0x0FF);
                                                    if (DEBUG) appendToLog("found inventory select mask length: " + byteArrayToString(dataInPayload)
                                            ...
                                            case 0x080C: {
                                                int dataIndex = mRx000Setting.invSelectIndex;
                                                if (dataIndex < mRx000Setting.INVSELECT_MIN || dataIndex > mRx000Setting.INVSELECT_MAX) {
                                                    if (DEBUG) appendToLog("found inventory select mask 0-3: " + byteArrayToString(dataInPayload) + ", but invalid index=" + dataIndex);
                                                } else {
                                                    int maskDataIndex = addressToRead - 0x0805;
                                                    if (DEBUG) appendToLog("Old selectMaskData0_31 = " + byteArrayToString(mRx000Setting.invSelectData[dataIndex].selectMaskData0_31));
                                                    ...
                                                    if (DEBUG) appendToLog("found inventory select mask 0-3: " + byteArrayToString(dataInPayload));
                                                }
                                                break;
                                            }
                                            case 0x0900:
                                                if (mRx000Setting.queryTarget != 2) mRx000Setting.queryTarget = (dataIn[startIndex + 4] >> 4) & 0x01;
                                            ...
                                            case 0x0901:
                                                ...
                                                if (DEBUG) appendToLog("found inventory configuration: " + byteArrayToString(dataInPayload) + ", algorithm=" + mRx000Setting.invAlgo + ", matchRep=" + mRx000Setting.matchRep + ", tagSelect=" + mRx000Setting.tagSelect + ", noInventory=" + mRx000Setting.noInventory + ", tagRead=" + mRx000Setting.tagRead + ", tagDelay=" + mRx000Setting.tagDelay);
                                                break;
                                            case 0x0902:
                                            ...
                                            case 0x0903: {
                                                ...
                                                if (dataIndex < mRx000Setting.ALGOSELECT_MIN || dataIndex > mRx000Setting.ALGOSELECT_MAX) {
                                                    if (DEBUG) appendToLog("found inventory algo parameter 0: " + byteArrayToString(dataInPayload) + ", but invalid index=" + dataIndex);
                                                } else {
                                                    ...
                                                    if (DEBUG) appendToLog("found inventory algo parameter 0: " + byteArrayToString(dataInPayload)
                                            ...
                                            case 0x0904: {
                                                int dataIndex = mRx000Setting.algoSelect;
                                                if (dataIndex < mRx000Setting.ALGOSELECT_MIN || dataIndex > mRx000Setting.ALGOSELECT_MAX) {
                                                    if (DEBUG) appendToLog("found inventory algo parameter 1: " + byteArrayToString(dataInPayload) + ", but invalid index=" + dataIndex);
                                                } else {
                                                    mRx000Setting.algoSelectedData[dataIndex].algoRetry = dataIn[startIndex + 4] & 0x0FF;
                                                    if (DEBUG) appendToLog("found inventory algo parameter 1: " + byteArrayToString(dataInPayload) + ", algoRetry=" + mRx000Setting.algoSelectedData[dataIndex].algoRetry);
                                                }
                                                break;
                                            }
                                            case 0x0905: {
                                            ...
                                            case 0x0911:
                                                ...
                                                if (DEBUG) appendToLog("found inventory match configuration: " + byteArrayToString(dataInPayload) + ", selectEnable=" + mRx000Setting.matchEnable + ", matchType=" + mRx000Setting.matchType + ", matchLength=" + mRx000Setting.matchLength + ", matchOffset=" + mRx000Setting.matchOffset);
                                                break;
                                            case 0x0912:
                                            case 0x0913:
                                            case 0x0914:
                                            ...
                                            case 0x0921: {
                                                ...
                                                if (DEBUG) appendToLog("found inventory match Data 0-3: " + byteArrayToString(dataInPayload));
                                                break;
                                            }
                                            case 0x0A01:
                                                mRx000Setting.accessRetry = (dataIn[startIndex + 4] & 0x0E) >> 1;
                                                if (DEBUG) appendToLog("found access algoRetry: " + byteArrayToString(dataInPayload) + ", accessRetry=" + mRx000Setting.accessRetry);
                                                break;
                                            case 0x0A02:
                                                mRx000Setting.accessBank = (dataIn[startIndex + 4] & 0x03);
                                                mRx000Setting.accessBank2 = ((dataIn[startIndex + 4] >> 2) & 0x03);
                                                if (DEBUG) appendToLog("found access bank: " + byteArrayToString(dataInPayload) + ", accessBank=" + mRx000Setting.accessBank + ", accessBank2=" + mRx000Setting.accessBank2);
                                                break;
                                            case 0x0A03:
                                                ...
                                                if (DEBUG) appendToLog("found access offset: " + byteArrayToString(dataInPayload) + ", accessOffset=" + mRx000Setting.accessOffset + ", accessOffset2=" + mRx000Setting.accessOffset2);
                                                break;
                                            case 0x0A04:
                                                mRx000Setting.accessCount = (dataIn[startIndex + 4] & 0x0FF);
                                                mRx000Setting.accessCount2 = (dataIn[startIndex + 5] & 0x0FF);
                                                if (DEBUG) appendToLog("found access count: " + byteArrayToString(dataInPayload) + ", accessCount=" + mRx000Setting.accessCount + ", accessCount2=" + mRx000Setting.accessCount2);
                                                break;
                                            case 0x0A05:
                                                ...
                                                if (DEBUG) appendToLog("found access lock configuration: " + byteArrayToString(dataInPayload) + ", accessLockAction=" + mRx000Setting.accessLockAction + ", accessLockMask=" + mRx000Setting.accessLockMask);
                                                break;
                                            case 0x0A08:
                                                mRx000Setting.accessWriteDataSelect = (dataIn[startIndex + 4] & 0x07);
                                                if (DEBUG) appendToLog("found write data select: " + byteArrayToString(dataInPayload) + ", accessWriteDataSelect=" + mRx000Setting.accessWriteDataSelect);
                                                break;
                                            case 0x0A09:
                                            case 0x0A0A:
                                            case 0x0A0B:
                                            ...
                                            case 0x0A17:
                                            case 0x0A18: {
                                                int maskDataIndex = addressToRead - 0x0A09; int maskDataIndexH = 0;
                                                if (mRx000Setting.accessWriteDataSelect != 0) maskDataIndexH = 16;
                                                for (int k = 0; k < 4; k++) {
                                                ...
                                                if (DEBUG) appendToLog("found access write data 0-3: " + byteArrayToString(dataInPayload));
                                                break;
                                            }
                                            case 0x0b60:
                                                mRx000Setting.currentProfile = dataIn[startIndex + 4];
                                                if (DEBUG) appendToLog("found current profile: " + byteArrayToString(dataInPayload) + ", profile=" + mRx000Setting.currentProfile);
                                                break;
                                            case 0x0c01:
                                                mRx000Setting.freqChannelSelect = dataIn[startIndex + 4];
                                                if (true) appendToLog("setFreqChannelSelect: found frequency channel select: " + byteArrayToString(dataInPayload) + ", channelSelect=" + mRx000Setting.freqChannelSelect);
                                                break;
                                            case 0x0c02:
                                                mRx000Setting.freqChannelConfig = dataIn[startIndex + 4] & 0x01;
                                                if (DEBUG) appendToLog("found frequency channel configuration: " + byteArrayToString(dataInPayload) + ", channelConfig=" + mRx000Setting.freqChannelConfig);
                                                break;
                                            case 0x0f00:
                                                ...
                                                if (DEBUG) appendToLog("found authenticate configuration: " + byteArrayToString(dataInPayload));
                                                break;
                                            case 0x0f01:
                                            case 0x0f02:
                                            case 0x0f03:
                                            case 0x0f04: {
                                                ...
                                                if (DEBUG) appendToLog("found authenticate match Data 0-3: " + byteArrayToString(dataInPayload));
                                                break;
                                            }
                                            case 0x0f05:
                                                ...
                                                if (DEBUG) appendToLog("found untraceable configuration: " + byteArrayToString(dataInPayload));
                                                break;
                                            default:
                                                if (true) appendToLog("found OTHERS with addressToWrite=" + addressToWrite + ", addressToRead=" + addressToRead + ", " + byteArrayToString(dataInPayload));
                                                break;
                                        }
                                        mRfidDevice.mRfidToWrite.remove(0); mRfidDevice.sendRfidToWriteSent = 0; mRfidDevice.mRfidToWriteRemoved = true;
                                    }
...
//                                    && mRfidToReading[startIndex + 6] == 0    //for packageTypeRead = 0x3007, this byte is 0x20. Others are 0
                                    && mRfidToReading[startIndex + 7] == 0) {  //if input as command response
                                {
                                    ...
                                    if (mRfidToReadingOffset - startIndex < expectedLength) break;
                                    dataIn = mRfidToReading;
                                    ...
                                    switch (packageTypeRead) {
                                        ...
                                        case 0x8000: //RFID_PACKET_TYPE_COMMAND_BEGIN  //original 0
                                            if (dataIn[startIndex + 0] != 1 && dataIn[startIndex + 0] != 2) {
                                                if (DEBUG) appendToLog("command COMMAND_BEGIN is found without first byte as 0x01 or 0x02, " + byteArrayToString(dataInPayload));
                                        ...
                                        case 0x8001:    //RFID_PACKET_TYPE_COMMAND_END  //original 1
                                            if (dataIn[startIndex + 0] != 1 && dataIn[startIndex + 0] != 2) {
                                                if (DEBUG) appendToLog("command COMMAND_END is found without first byte as 0x01 or 0x02, " + byteArrayToString(dataInPayload));
                                                break;
                                            } else {
                                        ...
                                        case 0x8005:    //RFID_PACKET_TYPE_18K6C_INVENTORY  //original 5
                                            if (dataIn[startIndex + 0] != 3 && dataIn[startIndex + 0] != 4) {
                                                if (DEBUG) appendToLog("command 18K6C_INVENTORY is found without first byte as 0x03, 0x04, " + byteArrayToString(dataInPayload));
                                                break;
                                            } else {
                                                if (dataIn[startIndex + 0] == 3) {
                                                    ...
                                                    if (true) {
                                                        boolean crcError;
                                                        if (dataA.dataValues.length < 12 + 4)   dataA.decodedError = "Received TYPE_18K6C_INVENTORY with length = " + String.valueOf(dataA.dataValues.length) + ", data = " + byteArrayToString(dataA.dataValues);
                                                        else {
                                                            int epcLength = (dataA.dataValues[12] >> 3) * 2;
                                                            if (dataA.dataValues.length < 12 + 2 + epcLength + 2)    dataA.decodedError = "Received TYPE_18K6C_INVENTORY with length = " + String.valueOf(dataA.dataValues.length) + ", data = " + byteArrayToString(dataA.dataValues);
                                                            else {
                                                                ...
                                                                if (true) dataA.decodedRssi = decodeWideBandRSSI(dataA.dataValues[13 - 8]);
                                                                else {
                                                    ...
                                                    if (true) {
                                                        if (dataA.dataValues.length < 3)   dataA.decodedError = "Received TYPE_18K6C_INVENTORY with length = " + String.valueOf(dataA.dataValues.length) + ", data = " + byteArrayToString(dataA.dataValues);
                                                        else {
                                                            int index = 0; byte[] dataValuesFull = dataA.dataValues;
                                                            while (index < dataValuesFull.length) {
                                                                ...
                                                                if (dataValuesFull.length >= index + 1) {
                                                                    if (true) dataA.decodedRssi = decodeWideBandRSSI(dataValuesFull[index]);
                                                                    else {
                                                                ...
                                                                if (false) appendToLog((dataA.dataValues != null ? "mRfidToRead.size() = " + mRfidDevice.mRfidToRead.size() + ", dataValues = " + byteArrayToString(dataA.dataValues) + ", " : "") + "2 decodedRssi = " + dataA.decodedRssi + ", decodedPc = " + byteArrayToString(dataA.decodedPc) + ", decodedEpc = " + byteArrayToString(dataA.decodedEpc));
                                                ...
                                                if (DEBUG) appendToLog("command 18K6C_INVENTORY is found with data=" + byteArrayToString(dataA.dataValues));
                                            }
                                            break;
                                        case 6: //RFID_PACKET_TYPE_18K6C_TAG_ACCESS
                                            if (dataIn[startIndex + 0] != 1) {
                                                if (DEBUG) appendToLog("command 18K6C_TAG_ACCESS is found without first byte as 0x02, " + byteArrayToString(dataInPayload));
                                                break;
                                            } else {
                                                ...
                                                if (true) {
                                                    ...
                                                    if (dataA.dataValues.length >= 8 + 12) {
                                                        ...
                                                        timeoutError = false;
                                                        crcError = false;
                                                        if ((dataA.dataValues[1] & 8) != 0)         crcError = true;
                                                        else if ((dataA.dataValues[1] & 4) != 0)    timeoutError = true;
                                                        else if ((dataA.dataValues[1] & 2) != 0)    backscatterError = (dataA.dataValues[13] & 0xFF);
                                                        else if ((dataA.dataValues[1] & 1) != 0 && dataA.dataValues.length >= 8 + 12 + 4)   accessError = (dataA.dataValues[20] & 0xFF) + (dataA.dataValues[21] & 0xFF) * 256;
                                        ...
                                        case 0x8007:    //RFID_PACKET_TYPE_ANTENNA_CYCLE_END    //original 7
                                            if (dataIn[startIndex + 0] != 1 && dataIn[startIndex + 0] != 2) {
                                                if (DEBUG) appendToLog("command TYPE_ANTENNA_CYCLE_END is found without first byte as 0x01 or 0x02, " + byteArrayToString(dataInPayload));
                                                break;
                                            } else {
                                                dataA.responseType = HostCmdResponseTypes.TYPE_ANTENNA_CYCLE_END;
                                                if (DEBUG) appendToLog("command TYPE_ANTENNA_CYCLE_END is found with packageLength=" + packageLengthRead + ", " + byteArrayToString(dataInPayload));
                                            }
                                            mRx000ToRead.add(dataA);
                                            break;
...
//                                                    dataIn[startIndex + 12] = 3;
                                                ...
                                                case 0x8E:
                                                    ...
                                                    if (DEBUG) appendToLog("spcialCountryVersion = " + mRx000OemSetting.spcialCountryVersion + ", data = " + byteArrayToString(dataInPart));
                                                    break;
                                                case 0x8F:
